...
**Budget.** Each run is capped at `max-ai-credits` (300), enforced by the firewall's API proxy, which prices every model request; hitting it skips the rest of the run rather than blocking. The daily cap (`max-daily-ai-credits`) is configured but **does not work with our BYOK model**: gh-aw's activation-side guardrail computes zero AI credits for these runs (verified on the drill pull request #46 — the agent ran with the cap set to 1), so it is not a bound anyone should rely on. Treat the per-run cap, the workflow `timeout-minutes`, and the concurrency group as the real limits; observed spend is a few cents per review (task 4.5 in the change records the reconciliation).
