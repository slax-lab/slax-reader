...
The repository has three long-lived environment branches: `dev`, `beta`, and `main` (the default branch). Changes flow `dev` → `beta` → `main` through promotion pull requests, and all three reject direct pushes. Promotion pull requests merge with **rebase**, never squash: rebase replays only the patches the target branch lacks, while squash forges a new commit per promotion and diverges the branches' histories.
