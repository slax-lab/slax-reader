...
| outcome | description |
|---|---|
| declined, reviews paused | `Review not run (reviews are currently paused)` — the pause is readable in the gate's own expressions as `vars.PR_REVIEW_ENABLED == 'false'`, the same evaluation GitHub performs for the review workflow's `if:` |
...
| activated, agent skipped | `Review not run (agent not started)` — shape only. The gate observes that activation succeeded and the agent did not start, never *why*: `daily_ai_credits_exceeded` is a job output, and the REST jobs API exposes conclusions, not outputs. The compiled `if:` makes the guardrail the only thing that can skip the agent (D2), but naming it would be an inference about gh-aw's internals rather than something read from the run — which is the one thing this rule refuses |
| failed before the agent | `Review did not start (<stage>: <conclusion>); failing closed`, with the single failed step's name in place of the conclusion when the jobs response carries exactly one, and the pre-agent stages' *outcomes* when no single stage stopped the run (D3) |

Reading the pause from `vars` costs no API call and is the one skip cause worth stating, because it is deliberate and an operator set it. It is also the one cause the gate reads *after* the fact rather than out of the run: the variable is evaluated when the gate runs, seconds after the observed run was declined, so the message is worded as the current setting ("reviews are currently paused") rather than as the reason that run stopped. A maintainer flipping the switch inside that window would otherwise be told a cause that is true now and may not have been true then; the wording makes the claim one the gate can always support, and it is recorded here because it is the one place the description rule is approximate in time rather than in kind. Closing it exactly would mean having the review workflow record the pause state in the run itself, which changes how reviews report in order to sharpen a message that can never be a false green. Reading the pull request's draft or fork state to name those too was considered and rejected: both are already visible on the pull request itself, and every extra inference is another claim that can be wrong — which is the failure this decision exists to remove.
...
- **The description could still mislead** if a stage fails for several reasons at once; it names one stage and at most one step, or the stages' outcomes when no single stage stopped the run, which is a claim the API supports — no cause is inferred from prose.
