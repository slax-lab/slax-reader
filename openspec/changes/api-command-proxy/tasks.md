...
## 10. 本地目录与完整 setup:api（用户明确要求）

- [x] 10.1 本地 api.toml/.env/.dev.vars 与生成配置归位 deploy/local，公开模板保留 Cloudflare 目录，更新全部默认路径、规则与文档。
- [x] 10.2 setup:api 只读检查用户准备的本地配置/开发密钥，缺失时报错（按用户最终要求撤回自动准备），统一执行完整后台启动，失败停止并显示原因。
...
- [x] 13.1 setup:api 校验后执行一次 Wrangler login，登录成功才初始化依赖，完成后退出；dev 独立启动 Workers。
- [x] 13.2 移除 dev:full 混合入口，保留 setup:backend 初始化别名，--check 不登录；同步错误码/信号回归、文档与规则。
