...
- No per-request error language. `MultiLangError.getMessage` resolves against a module-global language, so a frame written after a model call can be delivered in a language a concurrent request switched to. `createResponse` already has the same property for every non-streaming error, so this is existing infrastructure rather than something this path should fix alone — but the frame is where it becomes newly visible, so it is recorded. Related: the closing `closeChatStream` and the frame write use a local writer while the content helpers still share `this.wr`, so only termination and the error frame are guaranteed to reach the right stream.
...
- **Error frames on a partially streamed answer.** A failure after content has been delivered now adds an error tips bubble. It lands *above* the partial answer rather than beneath it, because both surfaces append the error straight to the message list while the partial answer is still buffered, and only flush that buffer when loading clears. The delta spec does not pin the order and the bubble is clearly attributable, but the ordering is worth revisiting if a surface later wants the error beneath the answer. Either way this is a visible behavior change on that path.
