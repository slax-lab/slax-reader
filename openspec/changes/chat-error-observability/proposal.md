...
- **Scope**: the browser extension's chat client (`apps/extension/src/components/Chat/chatbot.ts`) is included **only** for the streaming-path error-frame parse that the server's framing change requires — without it the extension would drop the frame and render nothing where it previously showed a message. The rest of that client's defects (no termination guarantee on read failure, un-timed waits) remain a follow-up change.
