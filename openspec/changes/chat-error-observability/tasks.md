...
- [ ] 7.2 Local end-to-end: induce a real provider failure (invalid key or cut the worker's egress) and confirm the page shows an error with a usable input instead of an infinite spinner. Not performed: the operator freed the ports, but the worker stack still cannot be started from a non-interactive session (the Vectorize bindings are `remote = true`, so `wrangler dev` needs a Cloudflare credential) and the chat route needs a session credential. The handler-level test (6.6) covers the same boundary deterministically; see `verification.md` for the full reasoning and the workerd probe result.
