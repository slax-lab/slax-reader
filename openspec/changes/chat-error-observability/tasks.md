...
- [x] 5.7 Gave the browser extension's chat client the same streaming-path envelope parse (`handleErrorLine`, called from both its streaming loop and its end-of-stream branch). The frame's framing makes it a complete mid-stream line, and that client previously parsed envelopes only at end of stream, so without this it would have rendered nothing where it used to show a message — a regression in a shipped client. The rest of that client's hardening stays out of scope (design D9).
...
- [x] 6.7 `apps/extension/tests/chatbot.test.ts`: a mid-stream envelope renders an error and clears loading, and an envelope seen only at end of stream still maps `NOT_SUBSCRIPTION` to its localized message. The mid-stream case was confirmed to fail with the extension's streaming envelope branch disabled.

## 7. Verification

- [x] 7.1 `pnpm api -- test`: 149 files passed, 0 failed (1653 tests). `pnpm api -- typecheck`: error set byte-identical with the change applied and stashed (120 pre-existing errors, all missing-generated-Prisma-client noise in untouched files); the clean main checkout reports 0, and none of the 120 touches a changed file. Web `chatbot.spec.ts`: 35/35. `pnpm extension -- test`: 5 files / 18 tests passed, `pnpm extension -- compile` clean, and the changed extension files add no lint problems. Plus the `web typecheck`, full-web-suite, and lint baseline comparisons recorded in `verification.md` and the PR body.
- [ ] 7.2 Local end-to-end: induce a real provider failure (invalid key or cut the worker's egress) and confirm the page shows an error with a usable input instead of an infinite spinner. **Server half done**: the real `AigcService` was run in workerd under local `wrangler dev` with a forced provider failure and returned the terminating envelope frame over real HTTP (see `verification.md`). The **browser half is not done**: the worker stack still cannot be started from a non-interactive session (the Vectorize bindings are `remote = true`, so `wrangler dev` needs a Cloudflare credential) and the chat route needs a session credential. The handler-level test (6.6) and the two client suites cover those boundaries deterministically, and a local browser pass could not serve as a negative control anyway (see `verification.md`).
- [ ] 7.3 Local end-to-end regression: normal streaming, tool progress, mermaid rendering and the subscription error path. Covered by automated tests (success stream, `[DONE]`, `NOT_SUBSCRIPTION` mapping, mermaid suite); the manual pass is deferred with 7.2.
- [x] 7.4 Run the three local pre-push passes defined in `REVIEW.md` (Bugs / Security / Compliance) and resolve Important findings before pushing. No Important findings in any pass; the nits and the compliance reconciliation are recorded in `verification.md`.
