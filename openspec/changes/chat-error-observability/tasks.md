...
- [x] 6.1 `apps/api/test/domain/aigcChatStream.test.ts`: provider failures of every class, an unexpected error, the validation exits (empty messages, unknown function call) and a throw before dispatch, each asserting the stream is closed and exactly one frame with the expected code is written. Note: the `!parts[0].functionCall` early return is unreachable (`isToolCallMessage` already requires every part to carry a function call); it is kept as a defensive throw rather than tested.
