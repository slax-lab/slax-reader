...
The v2 repository is the stable monorepo skeleton intended to make Slax Reader easier for non-programmers and third-party developers to understand and contribute to. The existing `slax_reader` repository already contains the DWeb application, the browser extension, and four workspace libraries used by both applications, but those projects are not yet present in v2.
...
- Add the four existing cross-app libraries under `packages/types`, `packages/types-pro`, `packages/utils`, and `packages/selection`.
