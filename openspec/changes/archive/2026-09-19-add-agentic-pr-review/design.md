...
The three reporting switches cut repository-issue noise but do not remove it: v0.88.7 has **no** setting that suppresses the failed-job path, which is why `agentic-noise-cleanup.yml` exists (7.16, verified in 8.3) and why `REVIEW.md` tells readers that `[aw] …` issues may still appear and are safe to close. `noop.report-as-issue` and `report-failure-as-issue` are documented per handler; v0.88.7 has no `threat-detection.report-as-issue` yet (the compiler rejects it), which is why the broader `report-failed-jobs: false` is used as well — task 1.8 measured the limit rather than a fix (a failing run still opened `[aw] Failed jobs: PR Review`, #19/#20/#21; the compiled env vars stay `true`), and the sweep is what keeps the repository clean. Both `[aw] No-Op Runs` and `[aw] Detection Runs` were created by the spike runs before any of this.
...
- Commit the Markdown source and its `.lock.yml` — **not** a centralized dispatcher: this repository's on-demand review is the inline `/review` trigger, the dispatcher that the abandoned single-workflow attempt generated was purged and no compile of the two-wrapper layout produces it (D3), and the drift check no longer lists that path (D11); run `gh aw init` once so `.gitattributes` marks compiled files correctly. Initialized with `gh aw init --engine copilot --no-mcp --no-skill --no-agent`, which touched only `.gitattributes` (adding `*.lock.yml linguist-generated=true`) — no extra scaffolding entered the repository (applied in task 1.2).
...
- **Reviewing untrusted content** from public PRs → AWF sandbox with read-only permissions, no secrets in the agent container, safe-outputs validation, threat detection on the agent's output, and `min-integrity: approved` applied automatically for public repositories. Detection **flags**, and the configuration this change ships makes a flag **block** (4.4, 8.5(b)): the run fails before `safe_outputs`, so a flagged output is never published and the merge gate fails closed. The cost is accepted deliberately — a false positive drops that review instead of showing it with a caution banner, and a re-review has to be requested. 8.6 is the drill run that shows both halves (the block in #59, the banner in #47).
