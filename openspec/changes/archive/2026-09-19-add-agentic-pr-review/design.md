...
- Commit the Markdown source and its `.lock.yml` — **not** a centralized dispatcher: this repository's on-demand review is the inline `/review` trigger, no `agentic_commands.yml` was ever produced here (D11), and the drift check no longer lists that path; run `gh aw init` once so `.gitattributes` marks compiled files correctly. Initialized with `gh aw init --engine copilot --no-mcp --no-skill --no-agent`, which touched only `.gitattributes` (adding `*.lock.yml linguist-generated=true`) — no extra scaffolding entered the repository (applied in task 1.2).
