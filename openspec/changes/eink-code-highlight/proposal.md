...
- Light and dark keep the incumbent Atom One Dark surface and syntax values, so the observable delta is confined to e-ink.
- Extend the theme-token guard tests: every theme must define the new tokens; the e-ink syntax palette must be grayscale and must clear a 4.5:1 contrast floor against the e-ink code surface. Add a source guard asserting no `highlight.js/styles/*` import remains in `apps/web`.
- **Not** included: the extension's chat surfaces (`apps/extension` has its own `atom-one-dark` import and no e-ink theme); the unreferenced `apps/web/app/components/Chat/{ChatBot,BubbleMessage}.vue`; styling the `code-block-header` label or the inert `code-block-header__copy` span; inline-code chips; and switching e-ink code blocks to wrapped lines instead of horizontal scroll.
