...
- **Pause switch**: a repository variable `PR_REVIEW_ENABLED` gates the whole workflow — automatic reviews and `/review` re-runs alike — so an operator can stop reviews for a day with one settings change: no commit, no recompile, no new run. While paused, automatic runs stay silent and the merge gate still reports a passing result so merges keep flowing, while a `/review` request is answered with a short notice that says the **automated review** is paused, that the pause is deliberate, and that human review is unaffected, so nobody has to guess whether the system is paused or broken — and nobody reads it as people being unable to review.
- **Budget guardrails**: an explicit per-run cap (`max-ai-credits`) bounds what a single runaway run can spend, enforced by the firewall's API proxy; hitting it skips the rest of the run rather than leaving the required check unreported. A daily cap (`max-daily-ai-credits`) is also configured, but verification showed it **cannot fire** for this BYOK model (task 2.6 — gh-aw's activation guardrail computes zero AI credits for these runs), so it is documented as inert rather than promised as a bound; deciding its replacement is task 8.5.
...
- **Cost**: DeepSeek tokens per review (off-peak $0.15/M cache-miss input, $0.6/M output) plus Actions minutes, which are free for this public repository. The per-run AI-credit cap bounds the worst case; the daily cap was intended to bound a busy day, but verification showed it cannot fire for this model (task 2.6), so it is not counted as a bound here — task 8.5 decides its replacement. Measured spend is recorded in task 4.5: 30 days of the review key cost ¥7.28, about 1.9× less than the credits the workflow accounted for.
