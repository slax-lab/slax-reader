...
- [ ] 2.7 Add the pause switch per design D10 (`if: vars.PR_REVIEW_ENABLED != 'false'`) and verify with the repository variable set to `false` that a newly opened pull request produces a skipped run with no agent execution, and with the variable unset or `true` that a run reviews normally
