...
1. Identify the pull request that triggered this run and read its **full** diff with the GitHub pull request tools. Those are the reliable source of the diff and of every changed file. Do not infer the worktree's shape: the checkout is the pull request's **head branch** on the comment path and the **merge commit** on the automatic one, so `git diff HEAD^` is the last commit in the first case and the base tip in the second; and `.github/`, `.agents/` and `AGENTS.md` are restored from the **base** branch, so the workflow and rulesync files you can read there are not the pull request's — read those paths through the tools. If you prefer git, resolve the range explicitly and check it against the tools' file list — for example `git diff "$(git merge-base origin/main HEAD)"..HEAD` when that base ref exists — and never review a partial diff silently: if you cannot obtain the whole diff, say so in the review instead of reviewing what you happen to have.
