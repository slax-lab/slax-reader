...
The repository has three long-lived environment branches: `dev`, `beta`, and `main` (the default branch). Changes flow `dev` → `beta` → `main` through promotion pull requests, and all three reject direct pushes.
