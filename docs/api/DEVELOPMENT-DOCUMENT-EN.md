...
- `deploy/local/.env`: Prisma database URLs and Cloudflare tool credentials (such as `CLOUDFLARE_API_TOKEN`) for D1/Wrangler, dev, deploy and resources --apply. Loaded automatically; D1 bindings still come from api.toml. Existing shell/CI values take precedence, including the local URLs pinned by setup:api. A missing default file allows CI variables or Wrangler login.
- `SLAX_API_ENV_FILE` selects an alternate file, absolute or relative to the repository root; explicit missing/unreadable files fail. The root .env is not discovered. Build, types, deploy --dry-run and resource plans do not load tool environment files.
